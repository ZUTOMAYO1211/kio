...
10. **`prune()` 은 장바구니에서 줄을 말없이 지운다.**
    `js/store.js` 의 `prune()` 이 config 커밋마다 돌면서, 사라졌거나 품절된 메뉴를
    장바구니에서 제거한다. `emit('cart')` 만 하고 무엇이 왜 빠졌는지 알리지 않아
    손님 화면에서 줄과 합계가 조용히 줄어든다. 재현된다. 판단은 `DECISIONS.md` 12절.

11. **스테이지가 스크롤 컨테이너가 되면 키오스크 전체가 밀려난다.**
